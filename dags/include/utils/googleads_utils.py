...
class extractReports:
    ...
    def getHeaders(self, accessToken):
        headers={'Authorization': 'Bearer {}'.format(accessToken), 'login-customer-id': '{}'.format(self.mccId), 'developer-token': '{}'.format(self.developerToken)}
        return headers
    def getAdAccounts(self, headers):
        def doGetAdAccounts(headers, **kwargs):
            ...
            if "results" in response:
                ...
                if "nextPageToken" in response:
                    ...
                    doGetAdAccounts(headers, fcontents=fcontents, nextPageToken=nextPageToken, accountList=accountList)
                else:
                ...
                fcontents += row
                s3Utils(self.config).writeToS3(fcontents,'dims/accounts/{}'.format(fname))
        response = doGetAdAccounts(headers)
        return response
    def getAdCampaigns(self, accountId, headers):
        def doGetAdCampaigns(accountId, headers, **kwargs):
            fcontents = kwargs.get('fcontents', '')
            nextPageToken = kwargs.get('nextPageToken', '')
            ...
            if "results" in response:
                ...
                if "nextPageToken" in response:
                    nextPageToken = response['nextPageToken']
                    doGetAdCampaigns(self.userId, personId, accountId, headers, fcontents=fcontents, nextPageToken=nextPageToken)
                else:
                ...
                fcontents += row
                s3Utils(self.config).writeToS3(fcontents,'dims/campaigns/{}'.format(fname))
        doGetAdCampaigns(accountId, headers)
    def getAdGroups(self, accountId, headers):
        def doGetAdGroups(accountId, headers, **kwargs):
            fcontents = kwargs.get('fcontents', '')
            nextPageToken = kwargs.get('nextPageToken', '')
            ...
            if "results" in response:
                ...
                if "nextPageToken" in response:
                    nextPageToken = response['nextPageToken']
                    doGetAdGroups(self.userId, personId, accountId, headers, fcontents=fcontents, nextPageToken=nextPageToken)
                else:
                ...
                fcontents += row
                s3Utils(self.config).writeToS3(fcontents,'dims/adgroups/{}'.format(fname))
        doGetAdGroups(accountId, headers)
    def getAds(self, accountId, headers):
        def doGetAds(accountId, headers, **kwargs):
            fcontents = kwargs.get('fcontents', '')
            nextPageToken = kwargs.get('nextPageToken', '')
            ...
            if "results" in response:
                ...
                if "nextPageToken" in response:
                    nextPageToken = response['nextPageToken']
                    doGetAds(accountId, headers, fcontents=fcontents, nextPageToken=nextPageToken)
                else:
                ...
                fcontents += row
                s3Utils(self.config).writeToS3(fcontents,'dims/ads/{}'.format(fname))
        doGetAds(accountId, headers)
    def getAdLabels(self, accountId, headers):
        def doGetAdLabels(accountId, headers, **kwargs):
            fcontents = kwargs.get('fcontents', '')
            nextPageToken = kwargs.get('nextPageToken', '')
            ...
            if "results" in response:
                ...
                if "nextPageToken" in response:
                    nextPageToken = response['nextPageToken']
                    doGetAdLabels(accountId, headers, fcontents=fcontents, nextPageToken=nextPageToken)
                else:
                ...
                fcontents += row
                s3Utils(self.config).writeToS3(fcontents, 'dims/adlabels/{}'.format(fname))
        doGetAdLabels(accountId, headers)
    def getKws(self, accountId, headers):
        def doGetKws(accountId, headers, **kwargs):
            fcontents = kwargs.get('fcontents', '')
            nextPageToken = kwargs.get('nextPageToken', '')
            ...
            if "results" in response:
                ...
                if "nextPageToken" in response:
                    nextPageToken = response['nextPageToken']
                    doGetKws(accountId, headers, fcontents=fcontents, nextPageToken=nextPageToken)
                else:
                ...
                fcontents += row
                s3Utils(self.config).writeToS3(fcontents, 'dims/kws/{}'.format(fname))
        doGetKws(accountId, headers)
    def getKwLabels(self, accountId, headers):
        def doGetKwLabels(accountId, headers, **kwargs):
            fcontents = kwargs.get('fcontents', '')
            nextPageToken = kwargs.get('nextPageToken', '')
            ...
            if "results" in response:
                ...
                if "nextPageToken" in response:
                    nextPageToken = response['nextPageToken']
                    doGetKwLabels(accountId, headers, fcontents=fcontents, nextPageToken=nextPageToken)
                else:
                ...
                fcontents += row
                s3Utils(self.config).writeToS3(fcontents,'dims/kwlabels/{}'.format(fname))
        doGetKwLabels(accountId, headers)
    def getAdPerformanceReport(self, accountId, headers):
        def doGetAdPerformanceReport(accountId, headers, startDate, endDate, **kwargs):
            fcontents = kwargs.get('fcontents', '')
            nextPageToken = kwargs.get('nextPageToken', '')
            ...
            url = 'https://googleads.googleapis.com/v17/customers/{}/googleAds:search'.format(accountId)
            fname = f'{self.hashString(self.userId)}:{self.hashString(self.personId)}:{accountId}:facts-ads:{startDate}:{endDate}'
            payload = {"query": query, "pageToken": nextPageToken}
            self.r_session.headers.update(headers)
            r = self.r_session.post(url, json=payload)
            response = r.json()
            if "results" in response:
                ...
                if "nextPageToken" in response:
                    nextPageToken = response['nextPageToken']
                    doGetAdPerformanceReport(accountId, headers, fcontents=fcontents, nextPageToken=nextPageToken)
                else:
        ...
        for i in range(delta.days):
            ...
            doGetAdPerformanceReport(accountId, headers, interval, interval)
    def getKwPerformanceReport(self, accountId, headers):
        def doGetKwPerformanceReport(accountId, headers, startDate, endDate, **kwargs):
            fcontents = kwargs.get('fcontents', '')
            nextPageToken = kwargs.get('nextPageToken', '')
            ...
            url = 'https://googleads.googleapis.com/v17/customers/{}/googleAds:search'.format(accountId)
            fname = f'{self.hashString(self.userId)}:{self.hashString(self.personId)}:{accountId}:facts-kws:{startDate}:{endDate}'
            payload = {"query": query, "pageToken": nextPageToken}
            self.r_session.headers.update(headers)
            r = self.r_session.post(url, json=payload)
            response = r.json()
            if "results" in response:
                ...
                if "nextPageToken" in response:
                    nextPageToken = response['nextPageToken']
                    doGetKwPerformanceReport(self.userId, personId, accountId, headers, fcontents=fcontents, nextPageToken=nextPageToken)
                else:
        ...
        for i in range(delta.days):
            interval = startDate + timedelta(days=i)
            doGetKwPerformanceReport(accountId, headers, interval, interval)
    def getClickPerformanceReport(self, accountId, headers):
        def doGetClickPerformanceReport(accountId, headers, startDate, endDate, **kwargs):
            fcontents = kwargs.get('fcontents', '')
            nextPageToken = kwargs.get('nextPageToken', '')
            ...
            url = 'https://googleads.googleapis.com/v17/customers/{}/googleAds:search'.format(accountId)
            fname = f'{self.hashString(self.userId)}:{self.hashString(self.personId)}:{accountId}:facts-clks:{startDate}:{endDate}'
            payload = {"query": query, "pageToken": nextPageToken}
            self.r_session.headers.update(headers)
            r = self.r_session.post(url, json=payload)
            response = r.json()
            if "results" in response:
                ...
                if "nextPageToken" in response:
                    nextPageToken = response['nextPageToken']
                    doGetClickPerformanceReport(self.userId, personId, accountId, headers, fcontents=fcontents, nextPageToken=nextPageToken)
                else:
        ...
        for i in range(delta.days):
            interval = startDate + timedelta(days=i)
            doGetClickPerformanceReport(accountId, headers, interval, interval)
    def getChangeEventReport(self, accountId, headers):
        def doGetChangeEventReport(accountId, headers, startDate, endDate, **kwargs):
            fcontents = kwargs.get('fcontents', '')
            nextPageToken = kwargs.get('nextPageToken', '')
            ...
            url = 'https://googleads.googleapis.com/v17/customers/{}/googleAds:search'.format(accountId)
            fname = f'{self.hashString(self.userId)}:{self.hashString(self.personId)}:{accountId}:chevs:{startDate}:{endDate}'
            payload = {"query": query, "pageToken": nextPageToken}
            self.r_session.headers.update(headers)
            r = self.r_session.post(url, json=payload)
            response = r.json()
            if "results" in response:
                ...
                if "nextPageToken" in response:
                    nextPageToken = response['nextPageToken']
                    doGetChangeEventReport(self.userId, personId, accountId, headers, fcontents=fcontents, nextPageToken=nextPageToken)
                else:
        ...
        for i in range(delta.days):
            interval = startDate + timedelta(days=i)
            doGetChangeEventReport(accountId, headers, interval, interval)
